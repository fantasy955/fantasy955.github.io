...
`align-items`规定了 弹性盒子中**每一行**内部元素在交叉轴如何对齐；

`justify-content`规定了弹性盒子每一行在父级元素的交叉轴上如何布局；
...
![image-20230327162801830](assets/image-20230327162801830.png)

https://codepen.io/fantasy995/pen/WNgPBpz

## CSS变量

可以在`:root`伪类中定义整个文档可以重复使用的css变量，然后借助媒体查询切换变量属性值，实现主题切换的目的。

### 作用域

CSS变量的作用域取决于它们定义的位置。

如果你在其他选择器（除了`:root`）中定义CSS变量，那么它们将具有局部作用域，只能在该选择器及其子元素中使用。例如：
...
```css
<div style="--main-color: #ff0000;">
  <p style="color: var(--main-color);">This text will be red.</p>
</div>
```

需要注意的是，虽然你可以在内联样式中给CSS变量赋值，但这并不是推荐的做法。通常，最好在样式表中定义和使用CSS变量，以便更好地组织和维护代码。
