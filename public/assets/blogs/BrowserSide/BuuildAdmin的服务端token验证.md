在BuildAdmin中，所有操作都需要登录才能进行，因此，在控制器初始化时，就实例化了Auth对象，查询请求中的token并进行登录验证。

BuildAdmin对token做了持久化处理，会保存到数据库中，这样重启服务也不会丢失发布的token。（在`config\buildadmin.php`中）
...
public function initialize()
    {
        ...
        if (!action_in_arr($this->noNeedLogin)) {  // 需要登录验证 的方法
            $this->auth->init($token);  // 初始化验证器
...
`app\common\library\token\driver\Mysql.php`
