...
在`VUE`文件外访问静态资源，静态资源必须放在`public`文件夹下才可访问。

**App.vue**可以用以下方式访问样式文件，因为是在预编译阶段，node可以访问本地文件。

![image-20220825172859731](assets/image-20220825172859731.png)

**main.js**无法使用通过axios访问，因为以及处在浏览器运行阶段，node.js没有映射public以外的文件。
